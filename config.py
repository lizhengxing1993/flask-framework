...
class Config(object):
    ...
    SESSION_REDIS = redis.Redis(host='127.0.0.1', port='6379', db=5)  # 用于连接redis的配置
    ...
    # 也可以一键配置
    # CACHE_REDIS_URL = "redis://localhost:6379/6"  #连接到Redis服务器的URL。示例redis: // user: password @ localhost:6379 / 2
    # 下面五个参数是所有的类型共有的
    ...
    PROJECT_DIR = os.path.realpath(os.path.join(PROJECT_APP_DIR, ".."))  # 当前目录的上一级目录
    LOG_PATH = os.path.join(PROJECT_APP_DIR, "logs")  # 拼接logs路径

    # sqlalchemy 配置
    SQLALCHEMY_ECHO = False  # 这个是记录打印SQL语句用于调试的, 一般设置为False, 不然会在控制台输出一大堆的东西
    SQLALCHEMY_COMMIT_ON_TEARDOWN = True  # 自动提交省去了每次 commit，添加数据对象后立马取 id 返回None 立马要取 id 的地方要先commit一下
