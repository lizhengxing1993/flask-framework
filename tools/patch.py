...
def model_to_dict(obj, available_relation_list=None, format_dt=str, fields=None):
    def _get_dict_data(columns):
        if fields:
            columns = filter(lambda x: x in fields, columns)
        out = dict(map(lambda c: (c, format_dt(getattr(obj, c)))
        if isinstance(getattr(obj, c), datetime.datetime) else (c, getattr(obj, c)), columns))
        return out
    ...
    for name, relation in mapper.relationships.items():
        ...
        if related_obj is not None:
            if relation.uselist:
                out[name] = [model_to_dict(child, available_relation_list) for child in related_obj]
            else:
...
def db_to_dict():
    def _wrapped(self, available_relation_list=None, format_dt=str, fields=None):
